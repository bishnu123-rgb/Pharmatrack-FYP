...
import {
    ...
    ArrowLeft, Calendar, Barcode, Tag, TrendingDown, ClipboardList, Eye, X
} from "lucide-react";
...
const Purchases = () => {
    ...
    // Deep Linking: Auto-add prefilled medicine from Alerts
    useEffect(() => {
        if (!loading && location.state?.prefilledMedicineId && medicines.length > 0) {
            ...
            if (med) {
                ...
                if (!exists) {
                    addNewBatchItem(med);
                    setSearchTerm(med.name); // Also pre-search for clarity
                }
            }
        }
                ...
                {activeView === "new" ? (
                                        ...
                                        .map(m => {
                                            ...
                                            return (
                                                <div key={m.medicine_id} className="space-y-2">
                                                    {/* Medicine Header - For New Delivery */}
                                                    <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between group">
                                                        ...
                                                        <button
                                                            onClick={() => addNewBatchItem(m)}
                                                            className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all shadow-sm"
                                                        >
                                                            <Plus size={12} /> New Batch
                                                        </button>
                                                    </div>

                                                    {/* Child Batches - For Restock */}
                                                    {medBatches.length > 0 && (
                                                        <div className="pl-4 space-y-1.5 border-l-2 border-slate-100 ml-4">
                                                            {medBatches.map(b => (
                                                                <div key={b.batch_id} className="p-3 rounded-xl bg-white border border-slate-50 flex items-center justify-between group hover:border-indigo-100 transition-all">
                                                                    ...
                                                                    <button
                                                                        onClick={() => addRestockItem(b)}
                                                                        className="p-1.5 bg-indigo-50 text-indigo-500 rounded-lg hover:bg-indigo-600 hover:text-white transition-all"
                                                                    >
                                                                        <Plus size={14} />
                                                                    </button>
                                                                </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex flex-col sm:flex-row items-center gap-4">
                            <h2 className="text-xl font-black text-slate-900 sr-only">Delivery History</h2>
                                ...
                                <table className="w-full text-left">
                                    <thead className="bg-slate-50 border-b border-slate-100">
                                        <tr>
                                            ...
                                            <th className="px-8 py-5 text-[10px] font-black tracking-widest uppercase text-slate-400">Processed On</th>
                                            <th className="px-8 py-5 text-[10px] font-black tracking-widest uppercase text-slate-400">Supplier Authority</th>
                                            <th className="px-8 py-5 text-[10px] font-black tracking-widest uppercase text-slate-400 text-right">Investment</th>
                                                ...
                                                <tr key={p.purchase_id} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="px-8 py-5">
                                                        <div className="flex items-center gap-2 text-slate-600">
                                                            ...
                                                            <span className="text-[10px] font-black text-slate-300 uppercase">{new Date(p.purchase_date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-8 py-5">
                                                        <div className="space-y-0.5">
                                                            <p className="text-sm font-black text-slate-800">{p.supplier_name}</p>
                                                            <p className="text-[10px] font-bold text-slate-400">Authorized Supplier</p>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="px-8 py-5 text-right">
                                                        <button
                                                            onClick={() => handleViewPurchaseDetails(p.purchase_id)}
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
                        <div className="px-6 py-5 border-b border-slate-50 flex items-center justify-between bg-white sticky top-0 z-10">
                            <div>
                                <h2 className="text-xl font-black text-slate-900 tracking-tight leading-tight transition-colors">Purchase Fact Sheet</h2>
                        ...
                        <div className="p-6 overflow-y-auto space-y-6">
                            <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-3xl border border-slate-100">
                                <div className="space-y-0.5">
                                    ...
                                    <p className="text-[10px] font-bold text-slate-500 leading-tight">{selectedPurchase.supplier_phone || 'Unlisted'}</p>
                                </div>
                                    ...
                                    {selectedPurchase.items.map((item, idx) => (
                                        <div key={idx} className="flex items-center justify-between p-3 bg-white rounded-2xl border border-slate-100 hover:border-indigo-100 transition-colors">
                                            <div className="flex-1 min-w-0 pr-4">
                                                ...
                                                <p className="text-[9px] font-bold text-slate-400 mt-0.5 uppercase tracking-widest italic">{item.dosage_form || 'Generic Form'} {item.strength ? `| ${item.strength}` : ''}</p>
                                            </div>
                                            <div className="text-right flex items-center gap-4">
                                                ...
                                                <div className="text-right min-w-[70px]">
                                                    <p className="text-[8px] font-black text-slate-300 uppercase">Subtotal</p>
