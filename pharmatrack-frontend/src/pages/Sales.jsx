...
import { Plus, Trash2, ShoppingCart, Loader2, CheckCircle, AlertCircle, Search, User, Phone, X, Filter, Sparkles, Receipt, ShieldAlert, Info, Eye, TrendingUp, Calendar, Tag } from "lucide-react";
...
const Sales = () => {
    ...
    useEffect(() => {
        if (selectedSale || submitting) {
            document.body.style.overflow = "hidden";
        } else {
            document.body.style.overflow = "auto";
        }
        return () => { document.body.style.overflow = "auto"; };
    }, [selectedSale, submitting]);
    ...
    const fetchData = async () => {
        try {
            ...
            // Only show unexpired batches with stock and active medicines in the selection panel
            const valid = (batchData || []).filter(b =>
    ...
    const addToCart = (batch) => {
        ...
        setError(""); // Clear error when item is added successfully
    };
    ...
    // Main Dashboard View - Corrected DOM Hierarchy for Print
    return (
        <>
            {/* Main Interactive UI */}
                    ...
                    <div className="flex flex-col items-center justify-center h-[70vh] text-center space-y-8 animate-in zoom-in-95 duration-500">
                        ...
                        <div className="flex gap-4 no-print">
                            <button
                                ...
                                className="px-10 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[1.5rem] font-black text-sm transition-all shadow-xl shadow-indigo-100 active:scale-95 no-print"
                            >
                                New Transaction
                            </button>
                            <button
                                onClick={() => window.print()}
                                className="px-10 py-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-[1.5rem] font-black text-sm transition-all flex items-center gap-2 no-print"
                            >
                        ...
                        {viewMode === "pos" ? (
                            ...
                            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                                ...
                                <div className="bg-white rounded-[2.5rem] premium-shadow border border-slate-100 overflow-hidden">
                                    ...
                                    {/* Pagination UI inside History View - Removed */}
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300 no-print">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
                        <div className="px-6 py-5 border-b border-slate-50 flex items-center justify-between bg-white sticky top-0 z-10">
                            <div>
                                <h2 className="text-xl font-black text-slate-900 tracking-tight leading-tight">Sale Record Detail</h2>
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-0.5">REF: #SAL-{selectedSale.sale_id.toString().padStart(6, '0')}</p>
                            </div>
                        ...
                        <div className="p-6 overflow-y-auto space-y-6">
                            <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-3xl border border-slate-100">
                                ...
                                <div className="text-right space-y-0.5">
                                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Sold By</p>
                                    <p className="text-sm font-black text-slate-800 leading-tight">{selectedSale.sold_by_name || 'Pharmacist'}</p>
                                    ...
                                    {(selectedSale.items || []).map((item, idx) => (
                                        <div key={idx} className="flex items-center justify-between p-3 bg-white rounded-2xl border border-slate-100">
                                            ...
                                            <div className="text-right">
                                                ...
                                                <p className="text-[8px] font-black text-slate-300 uppercase leading-none">Subtotal</p>
                                                <p className="text-xs font-black text-slate-900 font-mono">NPR {item.unit_price * item.quantity}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="px-6 py-5 border-t border-slate-50 bg-slate-50/50 flex items-center justify-between no-print">
                            <div>
                            ...
                            <button
                                ...
                                className="px-6 py-2.5 bg-slate-900 border border-slate-800 hover:bg-slate-800 text-white rounded-xl font-black text-[9px] uppercase tracking-widest transition-all flex items-center gap-2 shadow-xl shadow-slate-200 active:scale-95 no-print">
                                <Receipt size={14} /> Reprint Receipt
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {/* Always Available Printable Receipt (Correct Isolation) */}
            {lastSaleData && (
                <div id="printable-receipt" className="hidden print:block bg-white p-8 text-slate-900 border-0 shadow-none">
                    <div className="max-w-[400px] mx-auto space-y-6">
                        ...
                        <div className="text-center pt-8 opacity-50">
                            ...
                            <p className="text-[8px] font-bold text-slate-400 mt-1">Pharmacy Terminal Powered by PharmaTrack </p>
                        </div>
                    </div>
                </div>
            )}
