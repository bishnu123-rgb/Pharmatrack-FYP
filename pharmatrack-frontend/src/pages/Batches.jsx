...
const SearchableSelect = ({ options, value, onChange, placeholder, disabled, theme = "light" }) => {
    ...
    return (
        <div className="relative w-full">
            <div
                className={`w-full px-4 py-2.5 rounded-xl border flex justify-between items-center transition-all ${disabled ? 'opacity-50 cursor-not-allowed bg-slate-100 border-slate-200' : isDark ? 'bg-white border-slate-200 cursor-pointer focus-within:border-indigo-400 shadow-sm' : 'bg-slate-50 border-slate-200 cursor-pointer hover:border-indigo-400'}`}
                onClick={() => !disabled && setOpen(!open)}
            >
                <span className={`text-sm tracking-tight truncate pr-4 ${selectedOption ? (isDark ? 'text-slate-600 font-black uppercase text-[11px] tracking-widest' : 'text-slate-900 font-bold') : 'text-slate-400 font-bold'}`}>
            ...
            {open && (
                <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-2xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2">
                    <div className="p-2 border-b border-slate-50 bg-slate-50/50">
                        <div className="relative">
                            ...
                            <input
                                autoFocus
                                type="text"
                                className="w-full pl-9 pr-3 py-2 text-xs font-bold bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-indigo-400 shadow-sm"
                    ...
                    <div className="max-h-48 overflow-y-auto p-1 custom-scrollbar">
                        {filtered.length === 0 ? (
                            <div className="p-4 flex flex-col items-center text-center text-xs text-slate-400 font-bold">
                                <Package size={20} className="mb-2 opacity-50" />
                                No medicines found
                            </div>
                        ) : (
                            filtered.map(opt => (
                                <div
                                    key={opt.value}
                                    className={`px-3 py-2.5 text-xs font-bold rounded-xl cursor-pointer transition-colors ${String(opt.value) === String(value) ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`}
                                    onClick={() => {
                                        onChange(opt.value);
                                        setOpen(false);
                                    }}
                        ...
                        <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-3xl flex flex-col relative overflow-hidden">
                            ...
                            <form id="batchForm" onSubmit={handleSubmit} className="p-6 bg-white space-y-5 overflow-y-auto max-h-[70vh]">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div className="space-y-1 md:col-span-2 relative z-[70]">
            ...
            {scannerOpen && (
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <div className="bg-white rounded-[2.5rem] p-8 w-full max-w-sm flex flex-col items-center relative overflow-hidden shadow-2xl border border-slate-100/20">
                        <button onClick={() => setScannerOpen(false)} className="absolute top-5 right-5 p-2 text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition-all rounded-full bg-slate-50"><X size={18} /></button>

                        <div className="w-16 h-1.5 bg-indigo-100 rounded-full mb-8"></div>

                        <div className="relative w-56 h-56 rounded-[2rem] border-4 border-slate-50 overflow-hidden bg-slate-100/50 flex items-center justify-center shadow-inner group">
                            <div className="absolute top-0 left-0 w-full h-[3px] bg-rose-500 shadow-[0_0_25px_rgba(244,63,94,1)] animate-laser-scan z-10" />

                            <Barcode size={80} className="text-slate-300 opacity-40 group-hover:opacity-60 transition-opacity" />

                            {/* Scanner Targeting Corners */}
                            <div className="absolute top-4 left-4 w-6 h-6 border-t-[3px] border-l-[3px] border-indigo-500 rounded-tl-xl" />
                            <div className="absolute top-4 right-4 w-6 h-6 border-t-[3px] border-r-[3px] border-indigo-500 rounded-tr-xl" />
                            <div className="absolute bottom-4 left-4 w-6 h-6 border-b-[3px] border-l-[3px] border-indigo-500 rounded-bl-xl" />
                            <div className="absolute bottom-4 right-4 w-6 h-6 border-b-[3px] border-r-[3px] border-indigo-500 rounded-br-xl" />
                        </div>

                        <h3 className="text-2xl font-black tracking-tight text-slate-900 mt-8 mb-2">Awaiting Scan...</h3>
                        <p className="text-xs font-bold text-slate-500 text-center mb-6 leading-relaxed">Point your barcode hardware at the product packaging or type the digits below.</p>

                        <div className="relative w-full">
                            <ScanLine className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-400" size={16} />
                            <input
                                autoFocus
                                type="text"
                                placeholder="890..."
                                className="w-full pl-11 pr-4 py-4 text-center tracking-[0.3em] font-black text-indigo-600 bg-indigo-50 border-2 border-indigo-100 rounded-2xl focus:outline-none focus:border-indigo-400 focus:bg-white placeholder-indigo-300 transition-all font-mono"
