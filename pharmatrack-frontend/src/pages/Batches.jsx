...
import { Plus, Edit, Trash2, X, Calendar, DollarSign, Fingerprint, Loader2, AlertCircle, AlertTriangle, Search, Package, Eye, Barcode, ShieldCheck, FileDown, CheckCircle2, History, Zap } from "lucide-react";
import { getBatches, createBatch, updateBatch, deleteBatch, getMedicines } from "../services/api";
                                        ...
                                        <tr key={b.batch_id} className="group hover:bg-slate-50/50 transition-colors duration-300">
                                            <td className="px-8 py-5">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 shrink-0 bg-white rounded-xl border border-slate-200 flex items-center justify-center text-slate-400 shadow-sm group-hover:bg-slate-900 group-hover:text-white transition-colors duration-300">
                                                        <Fingerprint size={18} />
                                                    </div>
                                                    <div>
                        ...
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm flex flex-col relative overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="p-5 overflow-y-auto max-h-[60vh] space-y-4">
                                <div className="p-4 bg-indigo-50/50 rounded-xl border border-indigo-100 flex items-start gap-3">
                                    <div className="p-2 bg-indigo-600 rounded-lg text-white"><Package size={16} /></div>
                                    <div className="space-y-0.5">
                                        <p className="text-[9px] font-black text-indigo-600 uppercase tracking-widest">Medical Context</p>
                                        <p className="text-xs font-black text-slate-800">{viewingBatch.medicine_name}</p>
                                        <p className="text-[9px] text-slate-500 font-bold">Standard formulation.</p>
                                    </div>
                                </div>
                ...
                deleteConfirmId && (
                    <div className="fixed inset-0 bg-[#0f172a]/60 backdrop-blur-sm z-[120] flex items-start pt-32 justify-center p-4 animate-in fade-in duration-200">
                        <div className="bg-white rounded-[2rem] p-8 w-full max-w-sm shadow-2xl relative text-center">
                            ...
                            <p className="text-sm font-bold text-slate-500 mb-8 leading-relaxed">This batch will be safely removed from active view but kept for historical audits.</p>
